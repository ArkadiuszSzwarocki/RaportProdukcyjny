...
function renderItems() {
        ...
        updateSaveButtonState();
        updateActiveOrderBadges();
    }

function updateActiveOrderBadges() {
        const badgeElements = document.querySelectorAll('.order-item-badge');
        if (!badgeElements || badgeElements.length === 0) return;

        const addedTotals = {};
        if (typeof items !== 'undefined' && Array.isArray(items)) {
            items.forEach(function(item) {
                const pName = String(item.productName || item.nazwa || '').trim().toLowerCase();
                if (!pName) return;
                const weight = parseFloat(item.netWeight || item.unitsPerPallet || 0) || 0;
                addedTotals[pName] = (addedTotals[pName] || 0) + weight;
            });
        }
        ...
        badgeElements.forEach(function(badge) {
            const rawProd = badge.getAttribute('data-prod') || '';
            const targetQty = parseFloat(badge.getAttribute('data-target-qty')) || 0;
            const normProd = normStr(rawProd);

            let currentAddedKg = 0;
            for (let pKey in addedTotals) {
                const normKey = normStr(pKey);
                if (normKey === normProd || (normProd.length > 3 && (normKey.includes(normProd) || normProd.includes(normKey)))) {
                    currentAddedKg += addedTotals[pKey];
                }
            }
            ...
            if (currentAddedKg >= targetQty) {
                badge.style.background = '#dcfce7';
                badge.style.borderColor = '#86efac';
                badge.style.color = '#14532d';
                if (textSpan) textSpan.innerHTML = `<strong>${rawProd}</strong>: ${currentAddedKg.toFixed(1)} / ${targetQty.toFixed(1)} kg`;
                if (iconSpan) {
