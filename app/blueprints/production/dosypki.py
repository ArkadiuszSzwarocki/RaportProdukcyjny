...
def register_production_dosypki_routes(
    ...
    def dosypka_strona(plan_id):
        ...
        try:
            ...
            current_app.logger.error(f'Error in dosypka_strona: {e}', exc_info=True)
    ...
    def api_dosypki():
        ...
        try:
            ...
            role = str(session.get('rola', '')).strip().lower()
            result = []
            visible_roles = ('laborant', 'operator', 'pracownik', 'produkcja', 'lider', 'admin', 'zarzad', 'masteradmin')
            for r in rows:
