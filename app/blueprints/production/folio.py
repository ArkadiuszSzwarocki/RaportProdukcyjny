                ...
                if req_opak:
                    ...
                    cursor.execute("""
                        ...
                        WHERE nazwa = %s 
                          ...
                          AND IFNULL(lokalizacja, '') NOT IN ('', 'None', 'ZUZYTE', 'ZUŻYTE', 'DO_ZWROTU')
                          AND (IFNULL(lokalizacja, '') != 'Maszyna' OR DATE(updated_at) >= CURDATE())
                          AND id NOT IN (SELECT opakowanie_id FROM agro_plan_opakowania WHERE is_active = TRUE)
                        ORDER BY nazwa ASC
                    """, (req_opak['nazwa'],))
