...
def link_packaging():
    try:
        data = request.get_json()
        success, error = AgroWarehouseService.link_packaging_to_plan(data.get('opakowanie_id'), data.get('plan_id'), ilosc_pobrana=data.get('ilosc_pobrana'), user_login=session.get('login'))
...
def return_packaging():
    try:
        ...
        result = AgroWarehouseService.return_packaging_from_machine(data.get('opakowanie_id'), data.get('stan_po'), data.get('lokalizacja'), session.get('login'), is_partial=is_partial, print_label=print_label)
        extra = {}
