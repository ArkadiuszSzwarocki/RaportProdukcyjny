...
def api_surowce_w_produkcji_historia(tank_code):
    ...
    history_rows = AgroWarehouseService.get_production_tank_history(tank_code, limit=limit, linia=linia)
...
def confirm_move():
    try:
        ...
        success = AgroWarehouseService.confirm_delivery(ruch_id, worker_login, linia=linia, lokalizacja=lokalizacja, nr_partii=nr_partii, data_produkcji=data_produkcji, data_przydatnosci=data_przydatnosci)
        if not success:
...
def return_from_production():
    try:
        ...
        if not surowiec_id or ilosc <= 0:
            return (jsonify({'success': False, 'error': 'Nieprawidłowe dane'}), 400)
        worker_login = session.get('login')
        AgroWarehouseService.return_from_production(surowiec_id, ilosc, worker_login, plan_id=plan_id, linia=linia, komentarz=komentarz, ruch_produkcja_id=ruch_produkcja_id, lokalizacja=lokalizacja)
        return jsonify({'success': True})
    except Exception as e:
