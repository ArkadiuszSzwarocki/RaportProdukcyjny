    ...
    def execute(conn, cursor, data_planu, plans, session):
        ...
        for idx, plan in enumerate(plans, start=1):
            ...
            if sekcja == 'Agro':
                ...
                opakowanie_id = None
                if opakowanie_id_val not in (None, '', 'None'):
                    try:
                        opakowanie_id = int(opakowanie_id_val)
                    except (ValueError, TypeError):
                        name_str = str(opakowanie_id_val).strip()
                        cursor.execute("SELECT id FROM magazyn_opakowania WHERE LOWER(TRIM(nazwa)) = LOWER(%s) ORDER BY id DESC LIMIT 1", (name_str,))
                        op_row = cursor.fetchone()
                        if op_row:
                            opakowanie_id = op_row[0]
                        else:
                            from app.services.agro.agro_opakowania_service import AgroOpakowaniaService
                            try:
                                opakowanie_id = AgroOpakowaniaService.create_packaging(name_str, 0, lokalizacja='MOP01', linia='Agro')
                            except Exception as create_err:
                                current_app.logger.warning(f"Failed auto-creating packaging '{name_str}': {create_err}")
                                opakowanie_id = None
                    
                etykieta_id = None
                if etykieta_id_val not in (None, '', 'None'):
                    try:
                        etykieta_id = int(etykieta_id_val)
                    except (ValueError, TypeError):
                        name_str = str(etykieta_id_val).strip()
                        cursor.execute("SELECT id FROM slownik_etykiety_agro WHERE LOWER(TRIM(nazwa)) = LOWER(%s) ORDER BY id DESC LIMIT 1", (name_str,))
                        et_row = cursor.fetchone()
                        if et_row:
                            etykieta_id = et_row[0]
                        else:
                            cursor.execute("INSERT INTO slownik_etykiety_agro (nazwa) VALUES (%s)", (name_str,))
                            etykieta_id = cursor.lastrowid

                is_czyszczenie = 'czyszczenie' in produkt.lower()
                if typ_opakowania == 'worki' and not is_czyszczenie and (not opakowanie_id or not etykieta_id):
                    return False, f'Wiersz {idx}: Dla linii AGRO z workami wyznaczony worek (opakowanie) oraz etykieta są obowiązkowe!'

                nk_agro = max_seq_map_agro.get('Zasyp', 0) + 1
                max_seq_map_agro['Zasyp'] = nk_agro
                cursor.execute(
