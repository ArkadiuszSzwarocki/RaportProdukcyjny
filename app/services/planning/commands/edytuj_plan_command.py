...
class EdytujPlanCommand:
    ...
    def execute(conn, cursor, linia, pid, data, session):
        ...
        if linia.upper() == 'AGRO':
            sekcja_name = (before[3] or '').lower()
            if sekcja_name == 'workowanie':
                ...
                if not opakowanie_id_int or not etykieta_id_int:
                    return False, 'Dla zlecenia Workowanie w linii AGRO wyznaczony worek (opakowanie) oraz etykieta są obowiązkowe!', {}, 400
                ...
                if opakowanie_id is not None:
                    try:
                        opk_int = int(opakowanie_id) if opakowanie_id not in (None, '', 'None') else None
                    except Exception:
                        opk_int = None
                    if opk_int != before[11]:
                        updates.append('opakowanie_id=%s')
                        params.append(opk_int)
                        changes['opakowanie_id'] = {'before': before[11], 'after': opk_int}

                if etykieta_id is not None:
                    try:
                        etyk_int = int(etykieta_id) if etykieta_id not in (None, '', 'None') else None
                    except Exception:
                        etyk_int = None
                    if etyk_int != before[12]:
                        updates.append('etykieta_id=%s')
                        params.append(etyk_int)
                        changes['etykieta_id'] = {'before': before[12], 'after': etyk_int}
