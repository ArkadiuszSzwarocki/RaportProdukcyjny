...
class DashboardService:
    ...
    def get_agro_packaging_context(dzisiaj: date) -> Dict[str, Any]:
        ...
        try:
            ...
            # Fetch active packaging IDs (currently linked to an active plan) so we don't show active items
            active_opak_ids = set()
            conn_active = get_db_connection()
            try:
                cur_act = conn_active.cursor()
                cur_act.execute("SELECT opakowanie_id FROM agro_plan_opakowania WHERE is_active = TRUE")
                active_opak_ids = set(r[0] for r in cur_act.fetchall() if r[0])
            except Exception as e:
                print(f"Error fetching active packaging IDs: {e}")
            finally:
                conn_active.close()

            today_date = date.today()

            def _is_valid_packaging_item(item):
                if item['id'] in active_opak_ids:
                    return False
                if item.get('lokalizacja') == 'Maszyna':
                    up_at = item.get('updated_at')
                    if not up_at:
                        return False
                    if hasattr(up_at, 'date'):
                        return up_at.date() >= today_date
                    return str(up_at)[:10] >= str(today_date)
                return True
            ...
            if przydzielone_opakowanie_id:
                ...
                try:
                    ...
                    if row and row[0]:
                        ...
                        all_warehouse_packaging = [
                            ...
                            if item['nazwa'] == przydzielona_nazwa and _is_valid_packaging_item(item)
                        ]
                except Exception as e:
                ...
                all_warehouse_packaging = [item for item in all_warehouse_packaging if _is_valid_packaging_item(item)]
