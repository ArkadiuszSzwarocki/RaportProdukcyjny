...
def _create_tables(cursor):
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS osip_transfer_items (
            ...
            item_type VARCHAR(20) NOT NULL DEFAULT 'raw',
            requested_qty DECIMAL(10, 2) NOT NULL DEFAULT 0.00,
            loaded_qty DECIMAL(10, 2) NOT NULL DEFAULT 0.00,
            unit VARCHAR(10) NOT NULL DEFAULT 'kg',
            status VARCHAR(20) NOT NULL DEFAULT 'PLANNED',
            created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
            FOREIGN KEY (transfer_id) REFERENCES osip_transfers(id) ON DELETE CASCADE
        )
    """)
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS przestoje_produkcyjne (
            id INT AUTO_INCREMENT PRIMARY KEY,
            linia VARCHAR(20) NOT NULL DEFAULT 'AGRO',
            sekcja VARCHAR(50) NOT NULL DEFAULT 'Workowanie',
            plan_id INT NULL,
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS magazyn_zamowienia (
            id INT AUTO_INCREMENT PRIMARY KEY,
            items JSON NOT NULL,
            operator_login VARCHAR(100) DEFAULT NULL,
...
def _migrate_columns(cursor):
    ...
    # Usunięcie nieprawidłowych unikalnych indeksów na lokalizacji (lokacje takie jak MP01, ZUŻYTE, OSIP mogą zawierać wiele palet)
    for tbl in ["magazyn_surowce", "magazyn_agro_surowce", "magazyn_opakowania", "magazyn_agro_opakowania"]:
        prefix = tbl.replace('magazyn_', '')
        for idx_name in [f"uq_{prefix}_lokal", f"uq_{prefix}_nazwa_lokal"]:
            try:
                cursor.execute(f"ALTER TABLE {tbl} DROP INDEX {idx_name}")
