...
from tests.e2e.seed_e2e_plan import create_plan
...
def run():
    ...
    if not ready:
        ...
        lp = os.path.join(str(ROOT), 'tests', 'e2e', 'e2e_playwright_runner.log')
        with open(lp, 'a', encoding='utf-8') as lf:
            lf.write('Server not ready at ' + base + '\n')
        return
    log_path = os.path.join(str(ROOT), 'tests', 'e2e', 'e2e_playwright_runner.log')
    with open(log_path, 'w', encoding='utf-8') as lf:
        try:
            p = subprocess.run([os.sys.executable, 'tests/e2e/ui_test_playwright.py'], env=env, capture_output=True, text=True, timeout=300)
            lf.write('=== STDOUT ===\n')
