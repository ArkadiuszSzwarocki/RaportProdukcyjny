...
with open('a:/GitHub/RaportProdukcyjny/templates/ustawienia_index.html', 'r', encoding='utf-8') as f:
    content = f.read()

# Remove the tiny inline containers
content = re.sub(r'<!-- Terminal Emulator -->.*?<div id="verify-terminal-lines"></div>\s*</div>', '<!-- Terminal results will open in modal -->', content, flags=re.DOTALL)
content = re.sub(r'<!-- DB Stats Table Grid -->.*?<tbody id="db-stats-tbody"></tbody>\s*</table>\s*</div>', '<!-- DB Stats results will open in modal -->', content, flags=re.DOTALL)

# Add the master modal HTML right before </body> or script block if it exists
modal_html = '''
...
new_js = '''
        // MasterAdmin System Verification
...
content = re.sub(r'// MasterAdmin System Verification.*?}\s*// PWA Registration', new_js + '\n        // PWA Registration', content, flags=re.DOTALL)

with open('a:/GitHub/RaportProdukcyjny/templates/ustawienia_index.html', 'w', encoding='utf-8') as f:
    f.write(content)

print("Updated JS and HTML layout for big modal.")
