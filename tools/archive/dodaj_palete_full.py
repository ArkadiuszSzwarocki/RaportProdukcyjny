...
>     def dodaj_palete(plan_id):
          ...
          cursor.execute(f"SELECT sekcja, data_planu, produkt, data_produkcji, zasyp_id FROM {table_plan} WHERE id=%s",
 (plan_id,))
          plan_row = cursor.fetchone()
      
          if not plan_row:
              conn.close()
              return ('BĹ‚Ä…d: Plan nie znaleziony', 404)
          ...
          if input_data_produkcji:
              try:
                  ...
                  conn.close()
                  return ('BĹ‚Ä…d: NieprawidĹ‚owy format daty produkcji (oczekiwano RRRR-MM-DD)', 400)
          ...
          if plan_sekcja not in ('Workowanie', 'Czyszczenie'):
              ...
              try:
                  ...
                  pass
              return ('BĹ‚Ä…d: Paletki moĹĽna dodawaÄ‡ tylko do Workowania (bufora) lub Czyszczenia', 400)
      
          if waga_input <= 0:
              conn.close()
              return ('BĹ‚Ä…d: Waga musi byÄ‡ wiÄ™ksza od 0', 400)
          ...
          try:
              ...
              cursor.execute(
                  f"SELECT id, nr_palety FROM {table_pal} WHERE plan_id = %s AND COALESCE(status, '') = 'rezerwacja' OR
DER BY id ASC LIMIT 1",
                  (plan_id,),
              )
              ...
              if plan_produkt == 'Czyszczenie':
                  cursor.execute(f"SELECT skan_sscc FROM {table_plan} WHERE id IN (%s, %s) AND skan_sscc IS NOT NULL LI
MIT 1", (plan_id, plan_zasyp_id or -1))
              ...
              if reserved_row:
                  ...
                  cursor.execute(
                      f"UPDATE {table_pal} SET waga = %s, tara = 25, waga_brutto = 0, data_dodania = %s, status = 'do_p
rzyjecia', dodal_login = %s, nr_palety = %s, nr_plomby = COALESCE(%s, nr_plomby) WHERE id = %s",
                  ...
                  cursor.execute(
                      f"INSERT INTO {table_pal} (plan_id, waga, tara, waga_brutto, data_dodania, status, dodal_login, n
r_palety, nr_plomby) VALUES (%s, %s, 25, 0, %s, 'do_przyjecia', %s, %s, %s)",
              ...
              try:
                  if paleta_id:
                      cursor.execute(f"SELECT COUNT(*) FROM {table_pal} WHERE plan_id = %s AND id <= %s", (plan_id, pal
eta_id))
                      ...
                      try:
                          ...
                          col = cursor.fetchone()
                          if col:
                              cursor.execute(f"UPDATE {table_pal} SET nr_palety_lp = %s WHERE id = %s", (nr_palety_lp, 
paleta_id))
                      except Exception:
              ...
              try:
                  ...
                  cursor.execute(
                      "INSERT INTO palety_historia (paleta_id, linia, typ_palety, akcja, komentarz, user_login) VALUES 
(%s, %s, 'wyrob_gotowy', 'UTWORZENIE', %s, %s)",
                      (paleta_id, linia, f"Utworzono paletÄ™: {nazwa_do_historii}, waga: {waga_input} kg", user_login)
                  )
              ...
              def _async_print_label(paleta_id_local, nr_palety_local, app, usr_ip=None, usr_name=None):
                  with app.app_context():
                          try:
                              ...
                              cur2 = conn2.cursor()
                              label_data_local = prepare_pallet_label_data(cur2, paleta_id_local, linia, source_table='
workowanie')
                          except Exception as prep_err:
                              app.logger.error('Failed to prepare label data for paleta %s: %s', paleta_id_local, prep_
err)
                              ...
                              for copy_num in range(1, 3):
                                  try:
                                      ...
                                      app.logger.info(
                                          ...
                                          copy_num, nr_palety_local, ok, override_name or getattr(printer_local, 'print
er_name', None), override_ip or getattr(printer_local, 'printer_ip', None), print_msg
                                      )
                                  except Exception as single_err:
                                      app.logger.error('Print attempt failed for paleta %s copy %s: %s', paleta_id_loca
l, copy_num, single_err)
                              ...
                              app.logger.error('Unexpected error in async print thread for paleta %s: %s', paleta_id_lo
cal, err)
              # ---------------------------------------------
              if is_original_czyszczenie:
                  # Rozliczenie straty workĂłw dla czyszczenia
                  try:
                      ...
                      if opak_id:
                          ...
                          cursor.execute(
                              "UPDATE magazyn_opakowania SET stan_magazynowy = GREATEST(0, stan_magazynowy - %s) WHERE 
id=%s",
                              (bags_to_deduct, opak_id)
                          )
                          current_app.logger.info("Deducted %s bags for Czyszczenie from folia %s", bags_to_deduct, opa
k_id)
              ...
              try:
                  t = threading.Thread(target=_async_print_label, args=(paleta_id, nr_palety, app_obj, user_printer_ip,
 user_printer_name), daemon=True)
                  t.start()
              ...
              try:
                  current_app.logger.info(
                      'Dodano paletÄ™: plan_id=%s, waga=%s kg, data_produkcji=%s, uĹĽytkownik=%s',
                      plan_id,
                      waga_input,
                  ...
                  audit_log(
                      'DodaĹ‚ paletÄ™',
                      f'plan_id={plan_id}, produkt={plan_produkt}, waga={waga_input} kg, data_produkcji={selected_data_
produkcji}'
                  )
              except Exception:
                  pass
              ...
              conn.rollback()
              conn.close()
              return ('BĹ‚Ä…d: Nie udaĹ‚o siÄ™ dodaÄ‡ paletki', 500)
