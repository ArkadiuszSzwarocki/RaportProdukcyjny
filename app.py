...
from routes_planista import planista_bp  # <--- NOWY IMPORT
...
app.register_blueprint(planista_bp) # <--- NOWA REJESTRACJA
...
def login():
    if request.method == 'POST':
        ...
        if user: 
            ...
            # --- PRZEKIEROWANIE DLA PLANISTY ---
            if session['rola'] == 'planista':
                return redirect('/planista')
            # -----------------------------------
...
def index():
    ...
    try: 
        dzisiaj = datetime.strptime(data_str, '%Y-%m-%d').date() if data_str else date.today()
    except: 
        dzisiaj = date.today()
    ...
    if aktywna_sekcja in ['Zasyp', 'Workowanie']:
        cursor.execute("SELECT id, produkt, tonaz, status, TIME_FORMAT(real_start, '%H:%i'), TIME_FORMAT(real_stop, '%H:%i'), TIMESTAMPDIFF(MINUTE, real_start, real_stop), tonaz_rzeczywisty FROM plan_produkcji WHERE data_planu = %s AND sekcja = %s ORDER BY (real_start IS NULL) ASC, real_start ASC, id ASC", (dzisiaj, aktywna_sekcja))
        plan_dnia = cursor.fetchall()
        for p in plan_dnia:
            suma_plan += p[2] if p[2] else 0
            suma_wykonanie += p[7] if p[7] else 0
            if aktywna_sekcja == 'Workowanie':
                cursor.execute("SELECT waga, DATE_FORMAT(data_dodania, '%H:%i') FROM palety_workowanie WHERE plan_id = %s ORDER BY id DESC", (p[0],))
...
def wyslij_raport_email():
    ...
    dzisiaj = date.today()
    uwagi = request.form.get('uwagi_lidera', 'Brak')
    odbiorca = "zarzad@agronetzwerk.de"
    ...
    cursor.execute("SELECT sekcja, produkt, tonaz, tonaz_rzeczywisty FROM plan_produkcji WHERE data_planu=%s", (dzisiaj,))
    prod = cursor.fetchall()
    cursor.execute("SELECT sekcja, kategoria, problem, czas_start, czas_stop, TIMESTAMPDIFF(MINUTE, czas_start, czas_stop) FROM dziennik_zmiany WHERE data_wpisu=%s", (dzisiaj,))
    awarie = cursor.fetchall()
    cursor.execute("SELECT p.imie_nazwisko, o.typ, o.ilosc_godzin FROM obecnosc o JOIN pracownicy p ON o.pracownik_id=p.id WHERE o.data_wpisu=%s", (dzisiaj,))
    hr = cursor.fetchall()
    cursor.execute("SELECT sekcja, COALESCE(SUM(tonaz_rzeczywisty), 0) FROM plan_produkcji WHERE data_planu=%s GROUP BY sekcja", (dzisiaj,))
    sumy = cursor.fetchall()
...
def zamknij_zmiane():
    ...
    conn.commit()
    conn.close()
    flash("Zmiana zamknięta.", "success")
...
def raporty_okresowe():
    ...
    conn = get_db_connection(); cursor = conn.cursor()
    cursor.execute("SELECT COUNT(id), COALESCE(SUM(COALESCE(tonaz_rzeczywisty, tonaz)), 0), COALESCE(SUM(TIMESTAMPDIFF(MINUTE, real_start, real_stop)), 0) FROM plan_produkcji WHERE YEAR(data_planu)=%s AND MONTH(data_planu)=%s AND status='zakonczone'", (rok, mc))
    stats = cursor.fetchone()
    cursor.execute("SELECT kategoria, COUNT(id), COALESCE(SUM(TIMESTAMPDIFF(MINUTE, czas_start, czas_stop)), 0) FROM dziennik_zmiany WHERE YEAR(data_wpisu)=%s AND MONTH(data_wpisu)=%s GROUP BY kategoria", (rok, mc))
    awarie = cursor.fetchall()
    ...
    trend = cursor.fetchall(); conn.close()
...
def zarzad_panel():
    ...
    conn = get_db_connection(); cursor = conn.cursor()
    cursor.execute("SELECT COALESCE(SUM(tonaz), 0), COALESCE(SUM(tonaz_rzeczywisty), 0), COUNT(id) FROM plan_produkcji WHERE data_planu BETWEEN %s AND %s AND status='zakonczone'", (d_od, d_do))
    kpi = cursor.fetchone()
    cursor.execute("SELECT data_planu, SUM(tonaz), SUM(COALESCE(tonaz_rzeczywisty, 0)) FROM plan_produkcji WHERE data_planu BETWEEN %s AND %s GROUP BY data_planu ORDER BY data_planu", (d_od, d_do))
    ch = cursor.fetchall()
